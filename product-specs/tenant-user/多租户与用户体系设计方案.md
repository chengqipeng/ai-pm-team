...
| `paas_metarepo` | Tenant 级元数据 + 运行时 | p_tenant_entity, p_tenant_item, p_tenant_sharing_rule, p_tenant_data_permission, ... |
| **`paas_auth`** | **多租/用户/认证** | **p_passport, p_passport_log, p_user, p_totp_secret, p_tenant_password_policy, p_tenant_auth_provider, p_tenant_oauth_client, p_tenant_login_policy, p_tenant_ip_whitelist, p_tenant_third_binding** |
...
graph LR
    ...
    subgraph paas_metarepo["paas_metarepo"]
        TENANT_META["p_tenant_entity<br/>p_tenant_item<br/>p_tenant_sharing_rule<br/>p_tenant_data_permission<br/>..."]
    end
