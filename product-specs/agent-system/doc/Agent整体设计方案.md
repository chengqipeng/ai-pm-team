...
### 1.2 Agent 框架内部九层组件架构

Agent 框架内部按职责进一步细分为九层，对应 aPaaS 体系中第 4 层（核心引擎层）的内部结构：

Agent 框架内部按职责分为九层，自上而下形成清晰的依赖链路：
...
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │ 第 1 层 · 接入层（Gateway）                                        │  │
│  │   FastAPI + SSE 服务端 → 提供 /chat、/agui、/a2ui 三种接入模式     │  │
│  │   职责：协议适配、请求路由、流式响应、鉴权透传                      │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                              │ 请求下发                                  │
│                              ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │ 第 2 层 · 适配层（Adapter）                                        │  │
│  │   NeoAgentV2Adapter + AgentFactory                                 │  │
│  │   职责：Agent 实例管理、入口预处理（审查+改写）、执行模式分发        │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                              │ 创建/调度 Agent                           │
│                              ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │ 第 3 层 · 编排层（Orchestration）                                  │  │
│  │   状态机引擎：Router + PlanningNode + ExecutionNode + ReflectionNode│  │
│  │   职责：任务规划、步骤执行、反思决策、状态流转、检查点持久化        │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                              │ 洋葱模型拦截                              │
│                              ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │ 第 4 层 · 中间件层（Middleware Stack）                              │  │
│  │   20 个中间件按序组装（洋葱模型）                                   │  │
│  │   职责：横切关注点 — 追踪、日志、记忆、压缩、安全、审查、渲染       │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                              │ 能力调用                                  │
│                              ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │ 第 5 层 · 能力层（Capability）                                     │  │
│  │   三维度能力体系：Tool（原子操作）+ Skill（SOP 编排）+ Middleware（基础设施）│
│  │   职责：LLM 的手（Tool）、Agent 的 SOP（Skill）、系统的器官（Middleware）│
│  │   单向依赖：Skill → Tool → Middleware                              │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                              │ 委派任务                                  │
│                              ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │ 第 6 层 · 子 Agent 层（Multi-Agent）                               │  │
│  │   7 个业务域子 Agent（sales/service/analytics/config/data/research/general）│
│  │   职责：任务委派、工具裁剪、深度限制、同步/异步执行                 │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                              │ 数据存取                                  │
│                              ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────┐  │
...
│  │ 第 8 层 · 协议层（Protocol）                                       │  │
│  │   AG-UI 事件流 + A2UI 动态组件                                     │  │
│  │   职责：前端实时通信、组件渲染映射、多端适配                        │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                              │ 基础设施调用                              │
│                              ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │ 第 9 层 · 基础设施层（Infrastructure）                             │  │
│  │   LLM 客户端 + 模型路由 + 数据存储 + 链路追踪 + ID 生成            │  │
│  │   职责：外部服务接入、数据持久化、可观测性                          │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                                                                         │
├─────────────────────────────────────────────────────────────────────────┤
│                    外部依赖（Agent 框架之外）                              │
...
| 层级 | 组件 | 核心职责 | 上游依赖 | 下游提供 |
|:---|:---|:---|:---|:---|
| L1 接入层 | FastAPI Server | HTTP/SSE 协议适配、请求路由 | 前端客户端 | 请求对象 → 适配层 |
| L2 适配层 | Adapter + Factory | Agent 生命周期管理、预处理 | 接入层请求 | Agent 实例 → 编排层 |
| L3 编排层 | 状态机（Router + Nodes） | 任务规划、执行调度、反思决策 | Agent 实例 | 状态流转 → 中间件层 |
| L4 中间件层 | 20 个 Middleware | 横切关注点拦截与增强 | 编排层事件 | 增强后的状态 → 能力层 |
| L5 能力层 | Tool + Skill + Middleware | 原子操作、SOP 编排、基础设施 | LLM function calling | 执行结果 → 编排层 |
| L6 子 Agent 层 | 7 个业务域子 Agent | 专业领域任务委派 | 主 Agent 委派 | 执行结果 → 主 Agent |
| L7 数据层 | 记忆 + 知识库 + 压缩 | 长期记忆、RAG、上下文优化 | 中间件/能力层调用 | 上下文数据 → 编排层 |
| L8 协议层 | AG-UI + A2UI | 前端实时通信、组件渲染 | 编排层事件流 | SSE 事件 → 前端 |
| L9 基础设施层 | LLM Client + Store + Tracer | 外部服务接入、持久化 | 所有上层组件 | 基础能力 → 全框架 |

### 1.4 Agent 框架与 aPaaS 平台的交互边界

Agent 框架通过 **Tool 层** 与 aPaaS 平台交互，遵循严格的调用链路：
...
Agent 框架内部的数据流遵循"请求下行、响应上行"的模式：

```
用户请求 → 接入层 → 适配层 → 编排层 ←→ 中间件层
                                  ↓              ↓
                              能力层 ←→ 子Agent层
                                  ↓
                              数据层（记忆/知识库）
...
关键数据流:
  1. 用户消息 → 内容审查 → 查询改写 → 记忆召回 → LLM 推理
  2. LLM 决策 → Tool 调用 → aPaaS API → 业务数据返回
  3. 执行结果 → 上下文压缩 → 记忆提取 → 持久化
  4. 状态变更 → AG-UI 事件 → A2UI 组件映射 → 前端渲染
```

### 1.6 框架扩展点

Agent 框架提供五个核心扩展点，支持在不修改框架代码的前提下扩展能力：

| 扩展点 | 扩展方式 | 典型场景 | 热更新 |
|:---|:---|:---|:---:|
| Tool | 实现 Tool 接口 → 注册到 ToolRegistry | 新增业务操作（如"发送邮件"） | ✅ 支持 |
| Skill | 编写 SKILL.md → 放入技能目录 | 新增业务 SOP（如"客户入职流程"） | ✅ 支持 |
| Middleware（基础设施） | 实现 Middleware 接口 → 注册到 MiddlewareRegistry | 接入新数据源（如"天气 API"） | ✅ 支持 |
| 中间件 | 实现 Middleware Protocol → 加入 build_middleware() | 新增横切逻辑（如"限流中间件"） | ❌ 需重启 |
| 子 Agent | 配置 subagent_config → 指定工具集 | 新增业务域（如"HR Agent"） | ✅ 支持 |

### 1.7 核心边界原则

```
Agent 平台 = "大脑"（理解意图、规划执行、记住偏好、协调资源）
aPaaS 平台 = "身体"（定义模型、执行规则、保障数据、驱动流程）
统一服务   = "免疫系统"（认证身份、控制权限、审计合规、治理 API）
共同底座   = "循环系统"（多租户隔离、元数据驱动、微服务运行）
```

**判断任何能力归属的三问法**：

```
Q1: 这个能力需要 LLM 推理吗？        → 是 → Agent 平台
Q2: 这个能力操作业务数据/执行业务规则？ → 是 → aPaaS 平台
Q3: 这个能力是跨 AI 和 aPaaS 的横向治理？ → 是 → 统一平台服务
```
...
| 原则 | 体现 |
|:---|:---|
| 框架无关 | Router/Node/Middleware/Checkpointer 四个抽象可落到任意底层框架 |
| 单向依赖 | Skill → Tool → Middleware，逆向禁止；上层依赖下层，下层不感知上层 |
| 洋葱模型 | 中间件按序执行 before_step → Node 执行 → 逆序执行 after_step |
| 能力三问定位 | LLM 一次完成？→ Tool；多步推理？→ Skill；基础设施？→ Middleware |
| Agent 是调用者不是替代者 | Agent 通过 Tool 调用 aPaaS 组件 API，不拥有业务组件逻辑 |
| 数据正确性 > Agent 便利性 | aPaaS 的校验规则、权限规则是硬约束，Agent 不能绕过 |
| 优雅降级 | 任何非核心组件（审查/记忆/Middleware）失败不阻塞主流程 |
| 可观测全链路 | 从 LLM 调用到 aPaaS API 调用，全程 trace_id 贯穿 |
