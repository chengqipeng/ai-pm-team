...
| 外部调研 | research | web_search, web_fetch, company_info, financial_report, search_memories | 行业调研、企业尽调、政策查询 |
...
| 查外部信息 | web_fetch | "抓取这个网页" | — | search-plugin |
...
4. 总数: 16 个 Tool（查数据 3 + 管配置 3 + 查外部 4 + 协作 4 + 编排 2）
...
主 Agent 能看到全部 16 个 Tool，子 Agent 按业务域裁剪。以下是每个 Tool 在不同角色下的可见性：
...
**可见性规则**:
- 主 Agent 看到全部 16 个 Tool（Plugin 未启用的除外）
...
```json
{
  ...
  "required": false,
  "default_enabled": true,
  "description": "网络搜索 + 网页内容提取能力",

  "interface": {
    "query": "搜索关键词，返回结果列表（标题/URL/摘要）",
    "extract": "输入 URL 列表，返回页面正文内容"
  },

  "adapters": [
    {"name": "tavily", "description": "Tavily Search API（当前默认）", "supports": ["query", "extract"]},
...
对应的 Tool：web_search（调用 query）、web_fetch（调用 extract）。Plugin 未启用时两个 Tool 自动隐藏。
...
│   ├── search-plugin          — 网络搜索 + 网页提取（可选，供应商: Tavily/Bing/Google）
...
- [x] 实现网页提取工具 web_fetch（Tavily Extract API — 已调通）
...
| 工具体系 | 17 个 Tool（平台内置 + Plugin 依赖），7 个 Plugin | Tool 是 LLM 的稳定接口，Plugin 是可替换的供应商适配层 |
...
> - Tavily Extract API — `POST https://api.tavily.com/extract`（URL 内容提取）
