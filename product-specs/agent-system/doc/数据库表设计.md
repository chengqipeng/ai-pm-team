...
| 表 | 写入时机 | 写入方 | 实现状态 |
|---|---|---|---|
| ai_conversation | 首次对话时创建，每次消息后更新 message_count/total_tokens/last_message_at | TraceWriter._upsert_conversation() | ✅已实现 |
| ai_message | Agent 执行完成后写入（query + answer + trace_id + 执行统计） | TraceWriter._persist_message() | ✅已实现 |
| ai_message_ext | 有附件/反馈/卡片时写入 | FileProcessMiddleware / 前端反馈接口 | 📋待实现 |
| ai_trace | Tracer.start_trace 时创建（status=running），finish_trace 时更新 | TraceWriter.on_trace_start/on_trace_finish | ✅已实现 |
| ai_trace_span | trace 完成时批量写入所有 span | TraceWriter.on_trace_finish (batch_insert) | ✅已实现 |
| ai_content_review_log | 审查拦截时写入 | ContentReviewTransformer / OutputValidationMiddleware | ✅已实现 |
| ai_token_usage | trace 完成时按 trace 粒度写入（汇总所有 LLM 调用的 token） | TraceWriter._persist_token_usage() | ✅已实现 |

### 5.1 写入链路

```
用户发送消息 → /api/chat (server.py)
  │
  ├── tracer.start_trace()
  │     └── TraceWriter.on_trace_start() → INSERT ai_trace (status=running)
  │
  ├── Agent 执行（LangGraph astream_events）
  │     ├── on_chat_model_start/end → tracer 记录 llm_call span（内存）
  │     ├── on_tool_start/end → tracer 记录 tool_call span（内存）
  │     └── 中间件 spans → tracing_middleware 记录（内存）
  │
  └── 流结束后
        ├── tracer.finish_trace() → 合并所有 spans 到 trace 对象
        └── TraceWriter.on_trace_finish()
              ├── 1. batch_insert ai_trace_span（所有 span 一次性写入）
              ├── 2. UPDATE ai_trace（status/agent_output/tokens/duration）
              ├── 3. _upsert_conversation() → INSERT/UPDATE ai_conversation
              ├── 4. _persist_message() → INSERT ai_message（query+answer+trace_id）
              └── 5. _persist_token_usage() → INSERT ai_token_usage
```
