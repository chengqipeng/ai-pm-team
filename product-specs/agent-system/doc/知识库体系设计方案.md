...
| 检索引擎 | 全 VDB 多阶段召回（Phase1 并行 + Phase2 定向）+ 三维度归一化加权（α/β/γ） | `src/knowledge/retriever.py` |
...
知识库是独立子系统，采用 **Provider + Factory** 模式（无独立 Plugin 抽象层）。对外通过两个入口暴露能力：
...
> **架构更新（2026-05）**：已切换到**全 VDB 架构**，并优化为**多阶段召回**。
> - 路A 切片召回和路B 文档元数据召回**并行执行**（`asyncio.gather`），不再串行
> - 路B 的 ANN + BM25 合并为**一次 hybrid_search 调用**（`search_doc_metadata_hybrid`），不再两次独立调用
> - 新增 **Phase 2 定向召回**：有 filter 时用路B命中的 Top-50 doc_id 在切片集合内精确搜索
> - 切片原文（`content`）直接存入 VDB `kb_chunks`，命中后不再回 PG
> - 文档级集合从 `kb_doc_summary` 升级为 `kb_doc_metadata`，承载 summary 向量 + 5 路 BM25 稀疏 + γ 属性
> - α 维度归一化从 min-max 改为**理论锚点归一化**，分数含义更稳定
> - finalScore 映射到 `[threshold, 1.0]` 区间（`rawScore × (1-threshold) + threshold`）
...
**新架构**：所有检索信号都在 VDB，多阶段召回：
- Phase 1 并行：路A 切片 dense + sparse（`kb_chunks`）+ 路B 文档 ANN + BM25（`kb_doc_metadata`）
- Phase 2 定向：有 filter 时用路B Top-50 doc_id 在切片集合内精确搜索
- 文档摘要向量：`kb_doc_metadata.vector`
- 文档元数据 5 路 BM25：`kb_doc_metadata.sparse_vector`（title×3 + summary×2 + keywords×2 + candidate×1 + toc×1）
...
│                           │                                         │
│    ┌──── Step 1 ──────────┤                                         │
│    │ 查询改写 + 关键词提取 │ LLM 指代消解/省略补全 + jieba TF-IDF    │
│    └──────────────────────┤                                         │
│                           │                                         │
│    ┌──── Step 2 ──────────┤                                         │
...
│    │      文档级召回       │ (summary ANN + 加权BM25)           │    │
│    │                      │ 带元数据 filter                     │    │
│    └──────────────────────┴────────────────────────────────────┘    │
│                           │                                         │
│    ┌──── Step 3.5: Phase 2 ──────────────────────────────────┐      │
│    │ 定向召回（仅当有 filter 时触发）                          │      │
│    │ 用路B命中的 Top-50 doc_id → 在这些文档内搜切片            │      │
│    │ kb_chunks hybrid_search (doc_id IN [...])                │      │
│    └─────────────────────────────────────────────────────────┘      │
│                           │                                         │
│    ┌──── Phase 3 ─────────┤                                         │
│    │ 合并去重              │ 全局切片 ∪ 定向切片，按 score 降序      │
│    └──────────────────────┤                                         │
│                           │                                         │
│    ┌──── Step 4 ──────────┤                                         │
│    │ Hydrate + 扩展       │ VDB 直接返回 content，不回 PG           │
│    │                      │ Parent-Child 上下文扩展                  │
│    └──────────────────────┤                                         │
│                           │                                         │
│    ┌──── Step 5 ──────────┤                                         │
│    │ 三维度归一化加权      │ finalScore =                            │
│    │ + 切片→文档聚合      │   α·normA + β·normB + γ·normC           │
│    │                      │ α: 切片RRF聚合 (几何衰减)               │
│    │                      │ β: 路B融合排名 RRF                      │
│    │                      │ γ: VDB kb_doc_metadata 属性              │
│    └──────────────────────┤                                         │
│                           │                                         │
│    ┌──── Step 6 ──────────┤                                         │
...
**与前端调试界面的步骤映射**（`/api/knowledge/search-debug`）：

| 调试界面 Step | 对应链路 | 说明 |
|:---:|:---|:---|
| 1 | 用户问题 | 原始输入 |
| 2 | Step 1 查询改写 | LLM 改写 + 关键词提取 |
| 3 | Step 2 Self-Querying | 过滤条件提取 |
| 4 | Step 3 Phase 1 并行召回 | 切片 hybrid + 文档元数据 hybrid（asyncio.gather） |
| 5 | Step 3.5 Phase 2 定向召回 | 有 filter 时用路B结果定向搜切片 |
| 6 | Step 4 命中分片详情 | Hydrate + 上下文扩展 |
| 7 | Step 5 RRF 三维度加权排序 | α/β/γ 归一化加权 |
| 8 | Step 6 综合排序结果 | threshold 过滤 + top_k |
| 9 | 回答上下文 | 拼接 Top-N 分片供 LLM 回答 |

**关键架构决策**：
- Phase 1 的路A和路B是**并行执行**的（`asyncio.gather`），不是串行
- 路A 切片召回**不带元数据 filter**（全局召回），路B 文档元数据**带 filter**
- Phase 2 定向召回是**条件触发**的：仅当有 Self-Querying 提取的 filter 且路B有结果时执行
- 路B 的 ANN + BM25 是**一次 hybrid_search 调用**融合的，不是两次独立调用

### 5.2 Step 1 — 查询改写 + 关键词提取

**目的**：把多轮对话里的代词、省略句补全成独立可理解的 query，同时提取核心关键词。
...
**当前实现**：`_rewrite_query()` 方法已完整实现，使用 LLM 完成以下任务：
1. **指代消解**：将"它"、"这个"等代词替换为具体实体
2. **缩写展开**：如 "3051" → "罗斯蒙特3051压力变送器"
3. **语义增强**：添加同义词/相关术语提升召回率
4. **噪音去除**：去掉"请问"、"帮我"等无检索价值的词
5. **关键词提取**：3~8 个核心关键词，按重要性排序

**触发条件**：LLM 可用 **且** 有对话历史时触发。无对话历史时使用本地 jieba TF-IDF 提取关键词。

**降级**：LLM 调用失败时回退到本地 jieba 关键词提取，不阻塞后续流程。
...
召回分为 **Phase 1 并行** + **Phase 2 定向** + **Phase 3 合并** 三个阶段。

#### 5.4.0 整体流程

```
Phase 1: asyncio.gather 并行
  ├── [A] kb_chunks.hybrid_search（全局，不带元数据 filter）
  └── [B] kb_doc_metadata.hybrid_search（带元数据 filter）
          ↓
Phase 2: 定向召回（仅当有 filter 且路B有结果时触发）
  └── [A'] kb_chunks.hybrid_search（doc_id IN 路B Top-50）
          ↓
Phase 3: 合并去重
  └── chunk_results = 全局切片 ∪ 定向切片（按 score 降序，chunk_id 去重）
```

**设计意图**：
- 路A 全局召回**不带元数据 filter**，确保语义相关的切片不会因为打标不准而被漏掉
- 路B 文档元数据**带 filter**，用于 β 维度打分 + Phase 2 定向范围
- Phase 2 定向召回在路B命中的文档范围内精确搜切片，弥补路A全局召回可能遗漏的 filter 相关切片
- 最终合并去重，确保两路结果互补而不重复
...
#### 5.4.2b Phase 2 — 定向召回（条件触发）

**触发条件**：同时满足以下三个条件时执行：
1. Self-Querying 提取了有效的元数据过滤条件（`effective_filters` 非空）
2. 路B 文档元数据召回有结果（`doc_meta_rank` 非空）
3. Embedding 成功（`query_vec` 非空）

**路B 降级机制**：如果路B带 filter 返回 0 结果（文档未打标或打标值不匹配），自动去掉元数据 filter 重试路B。这确保：
- β 维度始终有打分数据（不会因为打标缺失导致 β=0）
- 降级后的 `doc_meta_rank` 仍可用于 Phase 2 定向召回

**目标**：在路B命中的高相关文档范围内，精确搜索切片。弥补路A全局召回（不带 filter）可能排名靠后的 filter 相关切片。

**实现**：
```python
target_doc_ids = doc_meta_rank[:50]  # 路B Top-50 文档 ID
targeted_filter = _build_chunk_filter_with_doc_ids(kb_id, target_doc_ids)
# → filter: kb_id=X AND doc_id IN ("doc1", "doc2", ...)
chunk_results_targeted = kb_chunks.hybrid_search(
    dense=query_vec, sparse=bm25(semantic_query),
    filter=targeted_filter, limit=max(top_k*3, 20)
)
```

**召回数量**：`max(top_k × 3, 20)`，默认 20 条。

**不触发时**：跳过，`chunk_results_targeted = []`。

#### 5.4.2c Phase 3 — 合并去重

将 Phase 1 全局切片和 Phase 2 定向切片合并：
- 按 `chunk_id` 去重（同一切片取较高 score）
- 按 score 降序排列

```python
chunk_results = merge_chunk_results(chunk_results_global, chunk_results_targeted)
```

#### 5.4.2d 安全机制 — Filter 降级重试

当合并后切片为空 **且** 有元数据过滤条件时，触发降级：
1. 去掉所有元数据 filter，重试路B文档元数据召回
2. 使用 Phase 1 的全局切片结果（本身就不带 filter）
3. 清空 `effective_filters`，后续打分不再考虑 filter

这确保了即使 Self-Querying 提取的 filter 过于严格，用户也能得到结果。
...
对所有文档的 `doc_rrf` 值做**理论锚点归一化**（非 min-max），映射到 `[0, 1]`：

```
theoretical_max_rrf = 1 / (K_CHUNK + 1)           # 排名第 1 时的理论最大值
theoretical_min_rrf = 1 / (K_CHUNK + chunk_count + 1)  # 排名最后时的理论最小值
rrf_range = theoretical_max_rrf - theoretical_min_rrf

normA = (doc_rrf - theoretical_min_rrf) / rrf_range
normA = clamp(normA, 0.0, 1.0)
```

- 最相关的文档 normA ≈ 1.0
- 最不相关的文档 normA ≈ 0.0
- 只有一个文档时 normA = 0.5（理论锚点中间值）

**与旧版 min-max 的差异**：理论锚点归一化不依赖当前批次的实际最大/最小值，避免了"只有一个文档时 normA=0.65"的不稳定行为。分数含义更稳定：同一个 doc_rrf 值在不同批次中得到的 normA 一致。

**finalScore 映射**：最终分数映射到 `[threshold, 1.0]` 区间：
```
finalScore = rawScore × (1 - threshold) + threshold
```
这确保通过 threshold 过滤的文档分数都 ≥ threshold。
...
```
rawScore 范围：
  最低：0.7 × 0.0 + 0.1 × 0 + 0.2 × 0 = 0.00
  最高：0.7 × 1.0 + 0.1 × 1.0 + 0.2 × 1.0 = 1.00
  典型：0.7 × 0.7 + 0.1 × 0.5 + 0.2 × 0.6 = 0.66

映射后 finalScore（threshold=0.3 时）：
  finalScore = rawScore × (1 - 0.3) + 0.3
  最低：0.00 × 0.7 + 0.3 = 0.30
  最高：1.00 × 0.7 + 0.3 = 1.00
  典型：0.66 × 0.7 + 0.3 = 0.762
```
...
Self-Querying: semantic_query="审批流怎么配置",
               filters={industryVertical:"制造业"}
```

#### 5.9.2 Step 3 — 多阶段召回

**Phase 1 路A**（切片混合，全局不带元数据 filter）：
```
...
**Phase 1 路B**（文档级 hybrid_search，带 filter={industryVertical:"制造业"}）：
```
...
**Phase 2 定向召回**（有 filter，用路B Top-3 doc_id 定向搜切片）：
```
chunk_01 (docA)  score=0.91  ← 与全局重复，取较高分
chunk_12 (docA)  score=0.87
chunk_55 (docC)  score=0.72  ← 全局未召回的新切片
```

**Phase 3 合并去重**：
```
chunk_01 (docA)  score=0.92  ← 取全局的较高分
chunk_12 (docA)  score=0.88
chunk_34 (docB)  score=0.81
chunk_55 (docC)  score=0.72  ← 定向召回补充
chunk_07 (docC)  score=0.65
chunk_45 (docA)  score=0.60
```

#### 5.9.3 Step 4-5 — 切片→文档聚合 + 三维度加权
...
```
# 6 个切片，K=60
theoretical_max = 1/(60+1) = 0.01639
theoretical_min = 1/(60+6+1) = 0.01493
rrf_range = 0.01639 - 0.01493 = 0.00146

normA_A = (0.02024 - 0.01493) / 0.00146 = clamp(3.64, 0, 1) = 1.000
normA_B = (0.01587 - 0.01493) / 0.00146 = 0.644
normA_C = (0.01562 - 0.01493) / 0.00146 = 0.473
```

**normB**（路B 融合排名 RRF × 0.15 / MAX）：
```
docA: rank=0 → (1/61+1/61)×0.15/0.00492 = 1.000
docB: rank=2 → (1/63+1/63)×0.15/0.00492 ≈ 0.968
docC: rank=1 → (1/62+1/62)×0.15/0.00492 ≈ 0.984
```
注：路B hybrid_search 已融合 ANN+BM25，同一排名列表同时作为 summary 和 meta_text 两路的 RRF 输入。
...
**rawScore → finalScore**（threshold=0.3）：
```
rawScore:
  docA = 0.7×1.000 + 0.1×1.000 + 0.2×0.794 = 0.959
  docB = 0.7×0.644 + 0.1×0.968 + 0.2×0.481 = 0.644
  docC = 0.7×0.473 + 0.1×0.984 + 0.2×0.600 = 0.550

finalScore = rawScore × (1-0.3) + 0.3:
  docA = 0.959 × 0.7 + 0.3 = 0.971  ← TOP
  docB = 0.644 × 0.7 + 0.3 = 0.751
  docC = 0.550 × 0.7 + 0.3 = 0.685
```

#### 5.9.5 Step 6 — threshold=0.3 过滤 + top_k=5

```
全部通过（均 > 0.3）→ 排序 → 取 top 5：
  chunk_01 (docA)  score=0.971
  chunk_12 (docA)  score=0.971
  chunk_55 (docC)  score=0.685  ← Phase 2 定向召回补充的切片
  chunk_45 (docA)  score=0.971
  chunk_34 (docB)  score=0.751
```
...
| **RRF k** | K_CHUNK / K_SUMMARY / K_META_TEXT | 60 | 排名→分数转换 |
| **聚合** | CHUNK_DECAY | 0.2 | 多切片几何衰减 |
| **归一化** | 理论锚点归一化 | — | α 维度使用 theoretical_max/min 锚点 |
| **映射** | finalScore 映射 | [threshold, 1.0] | rawScore×(1-threshold)+threshold |
