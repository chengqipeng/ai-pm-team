...
class KnowledgeRetriever:
    ...
    async def search(
        ...
        # Step 3 - 两路并行召回（B1+B2 合并为一次 hybrid_search）
        query_vec = await self._embed(semantic_query)
        chunk_results, doc_meta_hybrid = await asyncio.gather(
            self._recall_chunks(...),                # A: 切片 hybrid (dense+sparse)
            self._recall_doc_metadata_hybrid(...),   # B1+B2: 文档级 hybrid (summary ANN + BM25)
            return_exceptions=True,
        )
        # B1+B2 合并原因：tcvectordb 单集合只支持 1 dense + 1 sparse
        summary_doc_ids = meta_text_doc_ids = [r["id"] for r in doc_meta_hybrid]

        # Step 4 - Hydrate（VDB 直接返回 content，不回 PG）
        # Parent-Child 扩展走 VDB chunk_index 范围查询
...
| 表名 | 记录数 | 说明 |
|:---|:---|:---|
| ai_knowledge_base | 有数据 | 知识库定义 |
| ai_knowledge_dataset | 0 | 数据集分组（功能正常，暂无分组需求） |
| ai_knowledge_schema | 2 | 元数据 Schema |
| ai_knowledge_document | 6 | 文档主表 |
| ai_knowledge_chunk | 471 | 切片主表 |
| ai_knowledge_ingest_queue | 6 | 入库任务队列 |
| ai_knowledge_ingest_log | 6 | 入库进度日志 |
| ai_knowledge_search_log | 28 | 检索审计日志 |
...
# 2. 确认后执行清理
python scripts/clean_knowledge_data.py --commit
...
# 触发完整入库流水线：上传 → 解析(LKEAP) → 清洗 → 打标+评分 → 切分 → 索引(VDB)

# 5. 验证检索
python scripts/test_e2e_retrieval.py
```
...
| 解析 | `parse_status` 流转为 `done`，`page_count` / `total_chars` 有值 | LKEAP 解析成功 |
| 清洗 | `clean_status=done`，`quality_score > 0` | 文本清洗 + 质量评分完成 |
| 切分 | `chunk_status=indexed`，`ai_knowledge_chunk` 有对应切片 | 切片写入 PG |
| 索引 | `ai_knowledge_chunk.vector_synced=1` | 向量同步到 VDB 成功 |
| 检索 | `test_e2e_retrieval.py` 相关 query 命中 ≥1 条，无关 query 被过滤 | 端到端检索正常 |
