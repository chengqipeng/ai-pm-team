...
def build_seed_data() -> dict[str, list[dict]]:
    ...
    data["account"] = [
        {"id": "acc_001", "name": "华为技术有限公司", "industry": "通信设备", "city": "深圳",
         ...
         "description": "华为技术有限公司是全球领先的ICT基础设施和智能终端提供商，业务遍及170多个国家和地区。2024年实现营收8809亿元人民币，净利润680亿元，研发投入占比21.8%。当前战略重点包括5G基站建设、智能汽车解决方案（鸿蒙智行）、AI大模型（盘古大模型）和云计算服务。采购决策流程规范，IT部门评估后需经采购委员会审批，整个流程约3-6个月。对数字化转型有持续且强烈的投入意愿，年IT采购预算超过200亿元。",
         "recentNotes": "2025-04-15 张伟反馈：CRM项目已通过IT部门技术评审，下一步提交采购委员会。预计5月中旬有结果。建议准备ROI分析报告和竞品对比材料。2025-04-10 李娜确认：采购委员会每月第二周开会，需提前一周提交材料。2025-03-28 刘峰提到：BI平台项目可能与ERP升级合并为一个大项目，预算可能上调。"},
    ...
    data["opportunity"] = [
        ...
        {"id": "opp_001", "name": "华为ERP实施", "accountId": "acc_001", "amount": 45.0,
         ...
         "description": "华为ERP系统全面升级项目，覆盖财务、供应链、人力资源三大模块。客户现有SAP R/3系统已运行8年，面临性能瓶颈和功能缺失。项目分三期实施：一期财务模块（3个月）、二期供应链（4个月）、三期HR（3个月）。竞品Oracle和SAP S/4HANA也在竞标。",
         "nextSteps": "1. 提交技术方案终稿（4月25日前）2. 安排POC演示（5月第一周）3. 准备采购委员会汇报材料"},
        {"id": "opp_002", "name": "华为CRM部署", "accountId": "acc_001", "amount": 28.0,
         ...
         "description": "为华为全球销售团队部署CRM系统，支持5000+销售人员使用。需求包括：客户360视图、商机管理、销售预测、移动端支持。已通过IT部门技术评审，当前在采购委员会审批阶段。预计5月中旬关闭。",
         "nextSteps": "1. 等待采购委员会5月12日会议结果 2. 准备合同条款（已发法务审核）3. 确认实施团队排期"},
        {"id": "opp_003", "name": "华为BI平台", "accountId": "acc_001", "amount": 15.0,
         ...
         "description": "华为数据分析与商业智能平台建设，整合内部多源数据（ERP、CRM、供应链），提供管理驾驶舱和自助分析能力。刘峰提到可能与ERP升级合并为一个大项目。"},
        {"id": "opp_004", "name": "华为安全审计", "accountId": "acc_001", "amount": 18.0,
         ...
         "description": "已成交项目。为华为提供年度信息安全审计服务，覆盖网络安全、数据安全、应用安全三个维度。合同期1年，可续约。"},
        # 腾讯（2个商机）
    ...
    data["activity"] = [
        # 华为（8个活动，详细描述）
        {"id": "act_001", "type": "meeting", "subject": "华为ERP需求讨论会",
         "description": "与张伟（IT总监）深入讨论ERP实施范围。确认三期实施计划：一期财务（Q2）、二期供应链（Q3）、三期HR（Q4）。张伟强调必须支持多币种和跨国合并报表。会议决定：下周安排POC演示，重点展示财务模块的多币种处理能力。参会人员：张伟、刘峰、我方售前顾问王磊。",
        ...
        {"id": "act_002", "type": "call", "subject": "华为CRM报价跟进",
         "description": "与李娜（采购经理）确认CRM报价细节。李娜反馈：采购委员会对总价有异议，建议拆分为基础版+增值模块的报价方式。折扣方面，年付可给15%，三年期可给20%。李娜提醒：采购委员会5月12日开会，材料需5月5日前提交。",
        ...
        {"id": "act_003", "type": "email", "subject": "华为BI方案初稿发送",
         "description": "发送BI平台技术方案初稿给刘峰（项目经理）。方案包含：数据仓库架构设计、ETL流程、管理驾驶舱原型、自助分析模块。刘峰回复：方案整体可行，但需要增加实时数据流处理能力（Kafka集成），另外希望支持移动端看板。",
        ...
        {"id": "act_004a", "type": "meeting", "subject": "华为安全审计启动会",
         "description": "安全审计项目启动会。确认审计范围：网络安全（防火墙、VPN、入侵检测）、数据安全（加密、脱敏、备份）、应用安全（代码审计、渗透测试）。项目周期4周，第一周网络层，第二周数据层，第三周应用层，第四周报告编写。",
         "accountId": "acc_001", "opportunityId": "opp_004", "contactId": "con_001",
         "dueDate": "2025-04-05", "status": "completed", "createdAt": "2025-04-01 09:00:00"},
        {"id": "act_004b", "type": "task", "subject": "准备华为采购委员会材料",
         "description": "为5月12日采购委员会准备汇报材料：1.项目价值分析（ROI 3年回收）2.竞品对比（vs Salesforce/Oracle）3.实施风险评估 4.分期付款方案 5.成功案例（同行业3家）。需要市场部配合提供案例数据。",
         "accountId": "acc_001", "opportunityId": "opp_002", "contactId": "con_002",
         "dueDate": "2025-05-05", "status": "pending", "createdAt": "2025-04-20 10:00:00"},
        {"id": "act_004c", "type": "call", "subject": "华为ERP竞品情报收集",
         "description": "从渠道了解到Oracle已向华为报价，方案金额约5000万（含3年维护），实施周期18个月。SAP S/4HANA也在接触，但华为对SAP的本地化能力有顾虑。我们的优势：实施周期短（10个月）、本地化支持好、价格有竞争力。",
         "accountId": "acc_001", "opportunityId": "opp_001", "contactId": "con_003",
         "dueDate": "2025-04-12", "status": "completed", "createdAt": "2025-04-11 15:00:00"},
        {"id": "act_004d", "type": "meeting", "subject": "华为CRM POC演示",
         "description": "为华为IT团队演示CRM系统POC。演示内容：客户360视图、商机管理流程、销售预测AI模型、移动端体验。张伟评价：整体功能满足需求，但希望加强与企业微信的集成能力。刘峰关注数据迁移方案（从现有Siebel迁移）。",
         "accountId": "acc_001", "opportunityId": "opp_002", "contactId": "con_001",
         "dueDate": "2025-04-22", "status": "completed", "createdAt": "2025-04-20 14:00:00"},
        {"id": "act_004e", "type": "email", "subject": "华为ERP POC环境准备",
         "description": "向华为IT团队发送POC环境访问信息和测试数据说明。POC环境已部署在华为云上（按客户要求），包含财务模块完整功能。测试账号已创建，附带操作手册和视频教程。",
         "accountId": "acc_001", "opportunityId": "opp_001", "contactId": "con_003",
