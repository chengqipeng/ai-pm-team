...
#### ⛔ 强制计算分离规则（Agent 做判断，脚本做计算）

**以下情形必须使用 `scripts/` 而非 LLM 心算，无论技能复杂度如何：**

| 计算类型 | 示例 | 必须用脚本的原因 |
|:---|:---|:---|
| 超过 3 个变量的加权评分 | 健康度=使用×30%+关系×25%+商业×20%+支持×15%+价值×10% | LLM 多变量加权计算错误率>30% |
| 百分比/比率计算 | 转化率、增长率、占比、环比、同比 | LLM 除法幻觉高发区 |
| 时间序列趋势 | 连续 N 期数据的方向判断和斜率计算 | LLM 容易混淆时间窗口 |
| 排名/排序 | 多维度综合排名、Top-N 筛选 | LLM 排序不稳定 |
| 金额加总/汇总 | Pipeline 总金额、团队业绩汇总 | LLM 大数加法出错率>20% |
| 日期/周期计算 | 平均销售周期、超期天数 | LLM 日期算术不可靠 |

**判定规则：**
- 技能 Prompt 中出现以上任一计算需求 → 必须创建 `scripts/calc.py` 或在 `scripts/main.py` 中实现
- `analyze_data` 工具的 count/sum/avg 可替代简单聚合（单一 metrics），但多步计算仍需脚本
- 如果评估后认为 `analyze_data` 足够（如仅需 count+group_by），在 Prompt 中注明 "由 analyze_data 计算，Agent 禁止心算验证"

**Prompt 中的强制标注：**
技能 Prompt 的"数据纪律"段落必须追加：
```
6. 禁止心算数值 — 所有涉及除法、百分比、加权评分的计算必须通过 analyze_data 或 scripts/ 完成，Agent 只引用结果
```
...
#### ⛔ 输出格式锁定规则（Level A 及以上技能强制执行）

生成的 Skill Prompt 的"输出格式"章节必须包含以下格式锁定声明：

```markdown
# ⛔ 输出格式锁定（最高优先级，违反则输出不合格）

1. 章节顺序严格固定，不得调换/合并/拆分/新增章节
2. "分析摘要"章节必须恰好 3-5 句话，结论先行
3. 所有数值型结论必须来自工具调用或脚本计算，禁止 Agent 心算
4. 每条行动建议必须含 WHO（角色）/WHAT（动作）/WHEN（时间线）
5. 所有分析目标无论行业/规模/场景，报告格式完全一致，差异只体现在内容中
6. 末尾必须有"数据局限性"声明（时间范围 + 数据覆盖率 + 关键假设）
7. 置信度标注不可省略（每个核心结论必须有 🟢/🟡/🔴）
```
...
| 技能 Level | 格式锁定要求 |
|:---|:---|
| Level G（纯生成） | 软约束：建议遵循但可灵活调整 |
| Level Q（查询） | 中约束：章节顺序固定，格式可微调 |
| Level A/C/W/F | 硬约束：严格锁定，违反则不合格 |
