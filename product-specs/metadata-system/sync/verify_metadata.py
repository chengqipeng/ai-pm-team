...
from .config import (CORE_ENTITIES, OLD_ITEM_TYPE_CODES,
                     NEW_ITEM_TYPE_TO_BIZ_PREFIX, VIRTUAL_TYPES)
...
class MetadataVerifier:
    ...
    def _v9_prefix(self):
        ...
        for r in rows:
            itype = r['dbc_int1']
            dcol = r['dbc_varchar3']
            if itype is None or dcol is None:
                continue
            # 跳过虚拟字段
            if itype in VIRTUAL_TYPES:
                continue
            # 跳过固定列（name, id 等非 dbc_ 前缀）
            if not dcol.startswith('dbc_'):
                continue
            # 跳过 RELATION_SHIP + VARCHAR 覆盖模式（api_key 关联）
            if itype == 10 and r.get('dbc_int2') == 1:
                continue
            exp = NEW_ITEM_TYPE_TO_BIZ_PREFIX.get(itype)
            if exp is None:
                continue
            if not dcol.startswith(exp):
