...
前端提交的 JSON 值类型必须与 dataType 严格匹配，后端不做隐式转换。详见 [字段值全链路格式规范](../字段值全链路格式规范.md)。

| dataType | dbc 前缀 | JSON 值类型 | 示例 | 禁止 |
|---|---|---|---|---|
| 1(VARCHAR) | dbc_varchar | string | `"opt_level_a"` | 禁止传 number |
| 3(BIGINT) | dbc_bigint | **number** | `1714233600000` | 禁止传 `"1714233600000"` 字符串 |
| 4(DECIMAL) | dbc_decimal | **string** | `"99999.50"` | 禁止传 `99999.50` number（浮点精度丢失） |
| 5(TEXT) | dbc_textarea | string | `"长文本..."` | — |
| 6(SMALLINT) | dbc_smallint | **number 0/1** | `1` | 禁止传 `true`/`"true"` |
| 7(TEXT_ARRAY) | dbc_array | **string[]** | `["a","b"]` | 禁止传逗号分隔字符串 |
