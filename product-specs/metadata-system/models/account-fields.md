...
| 22 | level | 单选(2) | VARCHAR | dbc_varchar3 | 客户等级：A/B/C/D | <!-- 修正：文本域→单选 2026-04-28 -->
| 23 | parentAccountId | 上级客户 | 整数(5) | BIGINT | dbc_bigint3 | 上级客户关联 ID |
| 24 | industryId | 行业 | 单选(2) | VARCHAR | dbc_varchar15 | 所属行业 | <!-- 修正：文本域→单选 2026-04-28 -->

## 三、联系信息

| # | apiKey | 标签 | 类型 | 存储 | 数据库列 | 业务说明 |
|---|--------|------|------|------|---------|---------|
| 25 | fState | 省份 | 单选(2) | VARCHAR | dbc_varchar16 | 省份（级联地址） | <!-- 修正：文本域→单选 2026-04-28 -->
| 26 | fCity | 市 | 单选(2) | VARCHAR | dbc_varchar17 | 城市（级联地址） | <!-- 修正：文本域→单选 2026-04-28 -->
| 27 | fDistrict | 区 | 单选(2) | VARCHAR | dbc_varchar18 | 区县（级联地址） | <!-- 修正：文本域→单选 2026-04-28 -->
| 28 | longitude | 经度 | 实数(6) | BIGINT | dbc_bigint7 | GPS 经度 | <!-- 修正：关联→实数 2026-04-28 -->
| 29 | latitude | 纬度 | 实数(6) | BIGINT | dbc_bigint9 | GPS 纬度 | <!-- 修正：关联→实数 2026-04-28 -->
...
| # | apiKey | 标签 | 类型 | 存储 | 数据库列 | 业务说明 |
|---|--------|------|------|------|---------|---------|
| 36 | employeeNumber | 总人数 | 单选(2) | VARCHAR | dbc_varchar31 | 公司员工规模 | <!-- 修正：dbColumn dbc_bigint4→dbc_varchar31 2026-04-28 -->
| 37 | annualRevenue | 销售额 | 实数(6) | DECIMAL | dbc_decimal3 | 年销售额 | <!-- 修正：关联→实数 2026-04-28 -->
...
| 43 | score | 客户分值 | 整数(5) | BIGINT | dbc_bigint19 | 客户评分 | <!-- 修正：单选→整数 2026-04-28 -->
...
| 52 | highSeaAccountSource | 客户来源 | 单选(2) | VARCHAR | dbc_varchar12 | 公海客户来源渠道 | <!-- 修正：日期→单选 2026-04-28 -->
| 53 | highSeaStatus | 状态 | 单选(2) | VARCHAR | dbc_varchar21 | 公海状态 | <!-- 修正：文本域→单选 2026-04-28 -->
| 54 | claimTime | 认领日期 | 日期(7) | BIGINT | dbc_bigint11 | 从公海认领的时间 | <!-- 修正：dataType VARCHAR→BIGINT 2026-04-28 -->
| 55 | expireTime | 到期时间 | 日期(7) | BIGINT | dbc_bigint13 | 公海到期时间 | <!-- 修正：dataType VARCHAR→BIGINT 2026-04-28 -->
...
| 73 | visitUnvisitDay | 未拜访天数 | 计算(27) | BIGINT | dbc_bigint22 | 距上次拜访天数 | <!-- 修正：单选→计算 2026-04-28 -->
...
修正 itemType/dataType/dbColumn 与物理列类型不匹配的字段，消除 PostgreSQL 写入时的类型转换错误。

| apiKey | 修正内容 |
|---|---|
| level, industryId, fState, fCity, fDistrict | itemType: 文本域(4) → 单选(2) |
| highSeaAccountSource | itemType: 日期(3) → 单选(2) |
| highSeaStatus | itemType: 文本域(4) → 单选(2) |
| longitude, latitude | itemType: 关联(10) → 实数(6) |
| annualRevenue | itemType: 关联(10) → 实数(6) |
| score | itemType: 单选(2) → 整数(5) |
| visitUnvisitDay | itemType: 单选(2) → 计算(27) |
| claimTime, expireTime | dataType: VARCHAR → BIGINT |
| employeeNumber | dbColumn: dbc_bigint4 → dbc_varchar31 |
| 9 个废弃字段 | 软删除（activeDays, gradeLabel, nameInitial, nameLenCategory, wonRatioText, compositeGrade, processedName, srcFlg, newOppFlg 等） |
