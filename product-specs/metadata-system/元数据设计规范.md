...
所有 0/1 布尔字段（Java Integer 类型）遵循以下规则：

| 语义 | 命名模式 | 示例 |
|---|---|---|
| 是否启用某能力 | `enableXxx`（不加 Flg） | `enableTeam`、`enableHistoryLog`、`enableCheckRule` |
| 某种状态/标记 | `xxxFlg` | `deleteFlg`、`customFlg`、`requireFlg`、`activeFlg`、`hiddenFlg` |
| 具体能力描述 | 使用具体语义名 | `searchable`、`creatable`、`updatable`、`cascadeDelete` |

禁止规则：
- 禁止 `is` 前缀（Java Integer 类型不用 `is`）：❌ `isDefault` → ✅ `defaultFlg`
- 禁止 `enable` + `Flg` 重复：❌ `enableTeamFlg` → ✅ `enableTeam`
