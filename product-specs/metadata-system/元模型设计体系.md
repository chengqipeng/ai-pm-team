...
│   定义"有哪些类型的元数据"                                │
│   当前已注册 10 种实体相关元模型 + globalPickOption        │
│   db_table 指向各元模型的 Tenant 级独立快捷表              │
├─────────────────────────────────────────────────────────┤
│ 第二层：元模型字段定义（p_meta_item）                      │
│   定义每种元模型有哪些属性字段                              │
│   当前共 290 个字段定义，覆盖 10 种元模型                   │
...
| 表名 | 库 | 说明 |
|---|---|---|
| `p_meta_model` | Common | 元模型注册表，定义有哪些类型的元数据（10 种实体相关） |
| `p_meta_item` | Common | 元模型字段定义，定义每种元模型的 Java 字段名、db 列名、字段类型等（290 个） |
...
**当前数据：9 条关联关系，定义了 10 种元模型之间的完整层级结构。**
