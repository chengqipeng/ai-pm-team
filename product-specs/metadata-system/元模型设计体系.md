...
| `p_meta_item` | Common | 元模型字段定义，定义每种元模型的 Java 字段名、db 列名、字段类型等（227 个） |
...
| `duplicateRule` | 查重规则 | 数据查重策略 | 17 |
| `duplicateRuleCriteria` | 查重条件 | 查重规则的匹配条件 | 9 |
| `duplicateMatchingRule` | 查重匹配规则 | 模糊匹配规则 | 8 |
