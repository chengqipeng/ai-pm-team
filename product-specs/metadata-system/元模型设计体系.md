...
| `p_tenant_meta_log` | Tenant | 元数据变更日志（独立表结构，非大宽表。字段：id, tenant_id, entity_api_key, metadata_api_key, metamodel_api_key, trace_id, old_value, new_value, op_type, from_type, sync + BaseEntity 6 字段） |
...
**当前数据：66 种元模型（元模型定义层 4 种 + 元数据实例层 62 种，其中 50 种可用于 UI 配置）。**
...
| `enable_ui_config` | SMALLINT | 是否可用于画布 UI 组件数据源配置（默认 1，UI 组件类/系统内部类设为 0） |
| `metamodel_layer` | SMALLINT | 数据层级（0=元数据实例层，1=元模型定义层）。定义层的 4 个元模型（metaModel/metaItem/metaLink/metaOption）对应 `p_meta_*` 表 |
| `enable_data_source` | SMALLINT | 该元模型的实例是否可作为业务数据层数据源（默认 0，当前仅 entity=1）。详见 [画布数据源体系设计](../frontend-framework/画布数据源体系设计方案.md) |
...
| globalPickOption | 1 | 1 | p_tenant_global_pick_option | p_common_metadata | p_tenant_global_pick_option |
...
| List\<String\>（多选/多值） | dbc_array | 1~10 | 多选编码列表、多态引用实体列表。PG 使用 VARCHAR(300)[]，MySQL 使用 JSON。详见 [多选字段数组类型设计方案](多选字段数组类型设计方案.md) |
...
|---|---|---|---|---|---|
| metaModel_to_metaItem | metaModel | metaItem | metamodelApiKey | 阻止 | 元模型包含字段定义 |
| metaModel_to_metaLink | metaModel | metaLink | parentMetamodelApiKey | 阻止 | 元模型包含关联关系 |
| metaModel_to_metaOption | metaModel | metaOption | metamodelApiKey | 阻止 | 元模型包含选项值 |
| metaItem_to_metaOption | metaItem | metaOption | itemApiKey | 阻止 | 字段包含选项值 |
...
```
── 元模型定义层（metamodel_layer=1）──
metaModel（元模型定义）
├── metaItem（字段定义）    referItemApiKey = metamodelApiKey
│     └── metaOption（选项值）referItemApiKey = itemApiKey
├── metaLink（关联关系）    referItemApiKey = parentMetamodelApiKey
└── metaOption（选项值）    referItemApiKey = metamodelApiKey

── 元数据实例层（metamodel_layer=0）──
entity（对象）
  ├── item（字段）
  │     ├── pickOption（选项值）
...
| 3 | MULTI_SELECT | 多选 | VARCHAR(300)[] | dbc_array | 多选（PG 原生数组 `'{opt1,opt2}'`）。业务数据大宽表使用 dbc_array 列 |
...
| 7 | TEXT_ARRAY | 文本数组 | dbc_array | List\<String\> |
