...
+-----------------------------------------------------------------------+
| 第 4.5 层：资源点扣费层（AI 专属，新增）                                 |
|   统一接入 · 路由 · 限流 · 降级 · 计费                                   |
|   用量计量 / 资源点兑换 / 配额管控 / 账单中心 / 扣费拦截                 |
...
### 3.4.1 资源点扣费层（支撑能力层与基础设施平台之间）

> 位于支撑能力层（推理引擎/上下文工程/安全层等）与底层基础设施平台（大模型网关/基础原子能力平台）之间，对所有下行调用进行统一计量与扣费，实现 AI 能力的商业化闭环。
...
+-----------------------------------------------------------------------+
| 支撑能力层（跨三大维度共享）                                             |
|   推理引擎 / 上下文工程 / 业务语义 / AG-UI / Multi-Agent / 安全层       |
+-----------------------------------------------------------------------+
                                    |
                                    | 所有下行调用
                                    v
+-----------------------------------------------------------------------+
| 💰 资源点扣费层（Resource Credit Metering）                             |
|   统一接入 · 路由 · 限流 · 降级 · 计费                                  |
|   用量计量 / 资源点兑换 / 配额管控 / 账单中心 / 扣费拦截                 |
+-----------------------------------------------------------------------+
                                    |
...
| 模块 | 定位 | 核心能力 | 说明 |
|:---|:---|:---|:---|
| **用量计量** | 精确记录每次底层能力调用的资源消耗 | Token 数（输入/输出分计）/ API 调用次数 / 文档解析页数 / 音频转写时长 / 向量检索次数 / 图片生成张数 | 所有经过大模型网关和基础原子能力平台的调用均被计量 |
| **资源点兑换** | 将异构资源消耗折算为统一的"资源点" | 多能力统一折算规则（如 1K Token = X 资源点，1 页 PDF = Y 资源点）/ 不同模型差异化定价 / 动态调价 | 屏蔽底层能力的计费差异，上层只感知"资源点"一种货币 |
| **配额管控** | 租户 / 应用 / 用户三级配额体系 | 租户总额度 / 应用级配额隔离 / 用户级日/月限额 / 阈值预警 / 超额策略（拒绝/降级/审批） | 防止单一租户/应用/用户过度消耗 |
| **账单中心** | 资源消耗的全链路可视化 | 实时余额 / 消费明细（按时间/应用/能力/用户维度）/ 趋势分析 / 账单导出 / 成本归因 | 支撑运营决策与客户自助查询 |
| **扣费拦截** | 调用前校验余额，不足时降级或拒绝 | 余额预检 / 不足拒绝 + 友好提示 / 降级策略（切换低成本模型）/ 欠费通知 / 充值引导 | 中间件形态，请求到达底层前完成扣费判断 |

#### 调用链路（含扣费）

```
Agent 支撑能力层（推理引擎 / RAG / 上下文压缩 等）
    |
    | 发起底层能力调用请求
    v
资源点扣费层
    |-- 1. 身份识别：提取 tenant_id + app_id + user_id
    |-- 2. 余额预检：查询当前配额余额是否充足
    |       |-- 余额不足 → 执行降级策略或拒绝 + 通知
    |       +-- 余额充足 → 继续
    |-- 3. 资源点预扣：根据预估消耗冻结资源点
    |-- 4. 放行请求 → 调用大模型网关 / 基础原子能力平台
    |-- 5. 实际结算：根据实际消耗（如实际 Token 数）调整扣费
    +-- 6. 记录明细：写入计费流水表
    |
    v
大模型网关平台 / 基础原子能力平台
...
| 底层能力 | 计量单位 | 折算规则（示例） | 说明 |
|:---|:---|:---|:---|
| GPT-4o 推理 | Token | 1K input token = 2 点，1K output token = 6 点 | 按模型能力分级定价 |
| DeepSeek-V3 推理 | Token | 1K input token = 0.5 点，1K output token = 1.5 点 | 国产模型成本优势 |
| 文档解析 | 页 | 1 页 = 1 点 | 含 PDF/Word/PPT |
| OCR 识别 | 张 | 1 张 = 0.5 点 | 图片文字识别 |
| ASR 语音转写 | 分钟 | 1 分钟 = 2 点 | 实时/离线统一 |
| 向量 Embedding | 千字符 | 1K 字符 = 0.1 点 | 文本向量化 |
| 向量检索 | 次 | 1 次 = 0.05 点 | 相似度检索 |
| AI 生图 | 张 | 1 张 = 5 点 | 图片生成 |
...
| 关系方 | 交互方式 | 说明 |
|:---|:---|:---|
| 支撑能力层 → 扣费层 | 每次调用底层能力前经过扣费中间件 | 推理引擎调 LLM、RAG 调向量库等均需过扣费层 |
| 扣费层 → 大模型网关 | 余额校验通过后放行请求 | 扣费层不修改请求内容，仅做计量与拦截 |
| 扣费层 → 基础原子能力平台 | 同上 | 文档解析、OCR、ASR 等调用同样经过扣费 |
| 扣费层 → 统一平台服务 | 账单数据同步到统一审计/运营后台 | 支撑财务对账与运营分析 |
| 扣费层 → 前端 | 通过 AG-UI 实时推送余额变化 | 用户可感知当前消耗 |

#### 关键设计原则

```
1. 先扣后用：请求到达底层平台前完成余额校验，避免"用了不付"
2. 预扣 + 结算：先按预估冻结，实际完成后多退少补（应对 Token 流式输出场景）
3. 统一货币：所有异构能力折算为"资源点"，简化上层感知
4. 三级隔离：租户 → 应用 → 用户，任一级超额不影响其他
5. 优雅降级：余额不足时优先切换低成本模型，而非直接拒绝
6. 全链路可追溯：每笔扣费关联 trace_id，可追溯到具体的 Agent 会话和 Tool 调用
```
